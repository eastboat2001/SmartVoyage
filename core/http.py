"""
功能：提供 Web 服务通用中间件。
作用：统一处理 request id、异常兜底和 HTTP 访问日志。
实现方式：通过 FastAPI middleware 包装请求生命周期并记录耗时。
"""
...
from observability.request_context import clear_request_id, ensure_request_id, set_request_id
